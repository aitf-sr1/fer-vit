#!/usr/bin/env python3
"""Run training sequentially over multiple config files.

Usage:
    python scripts/train_sweep.py \\
        --configs src/vit/config/sweeps/farl_ce_weighted.yaml \\
                  src/vit/config/sweeps/farl_focal_g2.yaml \\
                  src/vit/config/sweeps/farl_asl.yaml \\
        --tags sweep-01 loss-ablation

Each config runs as a fully independent training job:
  - Its own W&B run (init/finish per config)
  - Its own checkpoint directory (from output.checkpoint_dir in config)
  - Extra --tags are merged into each run's tag list

If a config crashes, the error is printed and the sweep continues with the
next config so the rest of the runs are not lost.
"""
...
def run_sweep(config_paths: list[str], extra_tags: list[str]) -> None:
    ...
    for idx, config_path in enumerate(config_paths, start=1):
        ...
        try:
            ...
            print(f"  Checkpoint : {config.get('output', {}).get('checkpoint_dir', 'N/A')}")
            print()
